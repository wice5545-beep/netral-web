...
export async function GET() {
  try {
    ...
    return Response.json({ conversations: [], error: e?.message }, { status: 200 })
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { rows } = await db.query(
      `INSERT INTO "Conversation" ("id", "userId", "title", "model", "createdAt", "updatedAt") VALUES (gen_random_uuid()::text, $1, $2, $3, now(), now()) RETURNING *`,
    ...
    return Response.json({ error: 'Failed to create', detail: e?.message }, { status: 500 })
  }
}
...
export async function DELETE() {
  try {
    ...
    console.error('[DELETE /api/conversations]', e)
